...
if not(os.path.isdir(compare_dir)):
    print("Failed to find folder for comparison.")
    ...
    with open(compare_dir + "/rewards.json") as json_file:
...
while step < total_steps:
    ...
    for _ in range(num_steps):
        ...
        # Store data
        if penalize_on_death:
            storage.store(obs, action, reward-(done*3), done, info, log_prob, value)
        else:
